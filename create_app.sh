...
# ── Extraer autor de CORE_ID (parte antes del primer punto) ──────
AUTHOR_ID="${CORE_ID%%.*}"

# ── Solicitar metadatos adicionales de la app ─────────────────────
ask "Descripción de la app (p.e., Mi app para openfpgaOS): "
read -r APP_DESCRIPTION
[[ -z "$APP_DESCRIPTION" ]] && APP_DESCRIPTION="$APP_NAME for openfpgaOS"

ask "Versión inicial (p.e., 0.1): "
read -r APP_VERSION
[[ -z "$APP_VERSION" ]] && APP_VERSION="0.1"

ask "Fecha de lanzamiento (YYYY-MM-DD, ENTER para hoy: $(date +%Y-%m-%d)): "
read -r APP_DATE_RELEASE
[[ -z "$APP_DATE_RELEASE" ]] && APP_DATE_RELEASE="$(date +%Y-%m-%d)"
...
# ── app.conf — metadatos específicos de la app ───────────────────
cat > "$APP_DIR/app.conf" << APPCONF
# openfpgaOS app configuration — generated by create_app.sh
# Leído por scripts/customize.sh para generar dist/sdk/
APP_AUTHOR="${AUTHOR_ID}"
APP_SHORT="${APP_NAME}"
APP_PLATFORM="${PLATFORM}"
APP_DESCRIPTION="${APP_DESCRIPTION}"
APP_VERSION="${APP_VERSION}"
APP_DATE_RELEASE="${APP_DATE_RELEASE}"
APPCONF
